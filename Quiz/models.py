...
class Player(BasePlayer):
    # Experiment
    q_exp_fairness = models.IntegerField(
        initial=None,
        choices=list(range(11)),  # gar fair - sehr fair
        label='Wie fair fanden Sie es, dass in einem Wettbewerb der erste Platz für grüne Teilnehmer reserviert wurde?',
        widget=widgets.RadioSelectHorizontal()
    )

    q_exp_distribution = models.StringField(
        label='Im vorangegangenen Teil durften Sie über die Bonusverteilung für die Gewinner:innen entscheiden. Unter '
              'welchen Gesichtspunkten haben Sie diese Entscheidung getroffen?'
    )

    q_exp_correct1 = models.IntegerField(
        initial=None,
        label='Welchen Rang muss ein grüner Spieler mindestens haben, um den ersten Platz zu erhalten?',
        min=1, max=4
    )

    q_exp_correct2 = models.IntegerField(
        initial=None,
        label='Welchen Rang muss ein blauer Spieler mindestens haben, um den zweiten Platz zu erhalten?',
        min=1, max=4
    )
    ...
    q_n_experiment = models.IntegerField(label='An wie vielen Experimenten haben Sie (ungefähr) bereits teilgenommen?',
                                         max=500, min=0)

    q_math = models.FloatField(label="Was war Ihre letzte Mathenote (1,0 - 6,0)?", min=1.0, max=6.0)

    q_GPA_now = models.FloatField(label="Was ist Ihre aktuelle Durchschnittesnote bzw. die Ihres letzten Abschlusses?",
                                  min=1.0, max=6.0)

    q_abitur = models.FloatField(label="Was war die Abschlussnote Ihres letzten Schulabschlusses (1,0 - 4,0)?", min=1.0,
                                 max=6.0)
